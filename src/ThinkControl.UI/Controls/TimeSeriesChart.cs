...
public sealed class TimeSeriesChart : FrameworkElement
{
    ...
    public static readonly DependencyProperty ValuesProperty = DependencyProperty.Register(
        nameof(Values),
        typeof(IEnumerable<TimeSeriesPoint>),
    ...
    public static readonly DependencyProperty LiveWindowMinutesProperty = DependencyProperty.Register(
        nameof(LiveWindowMinutes),
        typeof(double),
        typeof(TimeSeriesChart),
        new FrameworkPropertyMetadata(60d, FrameworkPropertyMetadataOptions.AffectsRender));

    public IEnumerable<TimeSeriesPoint>? Values
    {
        get => (IEnumerable<TimeSeriesPoint>?)GetValue(ValuesProperty);
    ...
    public double LiveWindowMinutes
    {
        get => (double)GetValue(LiveWindowMinutesProperty);
        set => SetValue(LiveWindowMinutesProperty, value);
    }
    ...
    protected override void OnRender(DrawingContext dc)
    {
        ...
        Rect plot = new(
            LeftAxis,
            TopPadding,
            Math.Max(1, ActualWidth - LeftAxis - RightPadding),
            Math.Max(1, ActualHeight - TopPadding - BottomAxis));
        ...
        TimeSeriesPoint[] all = (Values ?? Array.Empty<TimeSeriesPoint>())
        ...
        else
        {
            ...
            {
                context.BeginFigure(Map(visible[0], plot, xMin, xMax, yMin, yMax), false, false);
                for (int i = 1; i < visible.Length; i++)
                    context.LineTo(Map(visible[i], plot, xMin, xMax, yMin, yMax), true, false);
            }
            geometry.Freeze();
    ...
    private (DateTimeOffset Min, DateTimeOffset Max) ResolveTimeRange(IReadOnlyList<TimeSeriesPoint> points)
    {
        ...
        if (last <= first)
            return (first - TimeSpan.FromMinutes(1), first + TimeSpan.FromMinutes(1));
    ...
    private (double Min, double Max) ResolveValueRange(IReadOnlyList<TimeSeriesPoint> points)
    {
        ...
        else
        {
            ...
            min -= pad;
            max += pad;
            if (IncludeZero && min < 0 && points.All(point => point.Value >= 0))
                min = 0;
        }
        ...
        Brush foreground)
    {
        ...
        TimeSeriesPoint nearest = points
            .OrderBy(point => Math.Abs(Map(point, plot, xMin, xMax, yMin, yMax).X - hover.X))
            .First();
        ...
        string text = string.IsNullOrWhiteSpace(nearest.Label)
            ? $"{time}  ·  {value}"
            : $"{nearest.Label}  ·  {time}  ·  {value}";

        FormattedText ft = CreateFormattedText(text, 10, foreground);
        double boxWidth = ft.Width + 16;
        double boxHeight = ft.Height + 10;
        double left = p.X + 10;
        if (left + boxWidth > plot.Right)
            left = p.X - boxWidth - 10;
    ...
    private static void OnValuesChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        if (chart._observableValues is not null)
            chart._observableValues.CollectionChanged -= chart.Values_CollectionChanged;
