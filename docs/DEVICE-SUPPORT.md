...
The UI deliberately does not promise a literal “cycles saved” count. Alpha.50 instead calculates threshold-only context that remains valid for named and custom pairs: top-end headroom, recharge-window width, omitted equivalent full-charge throughput and the share of a transparent >70% high-SOC reference band omitted by the cap. This is not a chemistry-specific wear curve. Real cycle/calendar aging also depends on chemistry, temperature, charge rate, depth of discharge and time. Firmware cycle count and ThinkControl health trend remain separate measurements.
