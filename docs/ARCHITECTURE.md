...
Alpha.52 makes the runtime ETA estimator consume the verified preservation stop threshold as its charging endpoint. `BatteryEtaSample.ChargeTargetPercent` defaults to 100, so generic/non-preservation systems retain full-charge behavior. On a verified 85% stop threshold, target energy is `FullWh × 0.85`; the estimator calculates only the remaining Wh to that target. A target change is treated like an estimator mode change: the rolling power/ETA state resets before warming up again, preventing an ETA calculated to 100% from being relabeled as an 85% ETA. The view model exposes this as `BatteryEtaToChargeTarget` and labels every Battery/Home/Compact surface with the real endpoint.
